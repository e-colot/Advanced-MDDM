...
On fig \ref{fig:state_estimation_wrong_A} the introduced error on the model ($A = 2 A_{\text{true}}$) makes the model unstable. The Kalman filter allows to maintain a stable state thanks to its feedback mechanism. \\
...
Fig \ref{fig:state_estimation_wrong_Rv} and Fig \ref{fig:state_estimation_wrong_Rn} show the effect of wrong process and measurement noise covariances. In both cases, the estimates are not significantly affected, but the estimated standard deviation is higher than with the true parameters. 
...
\begin{itemize}
    ...
    \item Th initial state covariance \(P\) affects the transient gains.
\end{itemize}
